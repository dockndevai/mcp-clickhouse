...
`package.json` already declares `"mcpName": "io.github.dockndevai/mcp-clickhouse"`, which the official registry uses to verify npm ownership. After this, clients can run the server with `npx -y mcp-clickhouse` (no local clone needed).
...
Smithery indexes GitHub MCP servers and can host/run them. Connect the GitHub repo at smithery.ai, or add a `smithery.yaml` describing the stdio command. Because this is an npm stdio server, the runtime command is `npx -y mcp-clickhouse`.
